...
from frame_show import frame_show
                        ...
                        if D < 150.0:
                            ...
                            if(key_1 not in objects_set or key_2 not in objects_set):
                                channel.basic_publish(exchange='',
												routing_key='hello',body = body_str)
                                objects_set.add(key_1)
                                objects_set.add(key_2)
                        else:
                            cv2.line(frame, (object_1[0], object_1[1]), (object_2[0], object_2[1]), (0, 255, 0),
									 thickness=1, lineType=8)
                            cv2.putText(frame, "{:.1f}".format(D), (int(mX), int(mY - 10)),cv2.FONT_HERSHEY_SIMPLEX, 0.55, (240, 0, 159), 2)
                            objects_set.clear()
                    else:
                        break
